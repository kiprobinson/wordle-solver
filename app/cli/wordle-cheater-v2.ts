...
import { getSortedWordListV2, RateWordCriteria, getEmptyRateWordCriteria, DEFAULT_WORD_LIST } from "../lib/word-list";
 ...
 */
const cheatAtWordleV2 = async ():Promise<void> => {
  let wordList = [...DEFAULT_WORD_LIST];
  const criteria: Required<RateWordCriteria> = getEmptyRateWordCriteria();
  
  criteria.easyMode = await CliUtils.askForBoolean('Are you playing in easy mode?');
...
const showBestGuesses = (wordList:string[], criteria:RateWordCriteria): {abort:boolean, newWordList: string[]} => {
  const sortedWordList = getSortedWordListV2(criteria, wordList);
  ...
  const numPossibleWords = newWordList.length;
  ...
  for(let i = 0; i < numPossibleWords && i < NUM_WORDS_TO_SHOW; i++)
    console.log(`${i+1}: ${sortedWordList[i].word}${sortedWordList[i].easyModeOnly ? '*' : ' '}  ${sortedWordList[i].score.toFixed(6).padStart(11, ' ')}`);
...
export default cheatAtWordleV2;
